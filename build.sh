...
# Fucking toolchains
GCC_32="${TTHD}/gcc32/bin/arm-eabi-"
CLANG="${TTHD}/proton-clang/"
...
DATEI=$(date +"%Y_%m_%d_%I_%M")
	...
	if [[ -f ${COMPILED_IMAGE} ]]; then
		...
		cd ${kernel_dir}
cp ${builddir}/${TARGET_IMAGE} ${kernel_dir}/anykernel


 cd anykernel
 zip -r9 lybkernel_surya_${DATEI}.zip *
 mv lybkernel_surya_${DATEI}.zip ../flashable
 rm Image.gz-dtb
 cd ..
 cd flashable
 curl -F document=@lybkernel_surya_${DATEI}.zip "https://api.telegram.org/bot$BOTTOKEN/sendDocument" \
        -F chat_id="$CHATID" \
        -F caption="[BOT] Build for Surya"
 cd ..
	...
	curl "https://api.telegram.org/bot$BOTTOKEN/sendMessage" \
        -F chat_id="$CHATID" \
        -F text="[BOT] Build for Surya FAILED"
	fi
}
parse_parameters "${@}"
make_image
